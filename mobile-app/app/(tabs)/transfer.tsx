...
export default function TransferScreen() {
  ...
  const transfer = async () => {
    try {
      const res = await api<{ data: { receipt: object; newBalance: number } }>('/user/transfer', {
        method: 'POST',
        body: JSON.stringify({
      ...
      Alert.alert(
        'Transfer Successful',
        `New balance: PKR ${res.data.newBalance.toLocaleString()}`
      );
      setAmount('');
    } catch (e) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <TextInput style={styles.input} value={toAccount} onChangeText={setToAccount} placeholder="BAH..." />
...
const styles = StyleSheet.create({
  ...
  benRow: { backgroundColor: '#fff', padding: 16, borderRadius: 12, marginBottom: 8 },
